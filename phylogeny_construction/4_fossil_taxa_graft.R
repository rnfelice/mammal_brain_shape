...
pairs_D <- data.frame(
    bind = c(setdiff(species, merged_tree_C$tip.label)),
    reference = c(
        "Loris_tardigradus-Lemur_catta", # Adapis_parisiensis
...
merged_tree_D <- tree.merger(
    backbone = merged_tree_C, data = pairs_D2,
    tip.ages = tip.ages_D,
    plot = F
)
